// authController-fixed.js - Authentication controller with userId validation fix
...
/**
 * Get authenticated user profile - FIXED VERSION
 */
const getProfile = async (req, res) => {
  try {
    ...
    // Get user details using the numeric userId
    const userResult = await pool.request()
      .input('userId', sql.Int, userIdNumber) // Use the converted numeric value
      .query(`
...
const changePassword = async (req, res) => {
  try {
    ...
    // FIXED: Ensure userId is a number
...
const resetPassword = async (req, res) => {
  try {
    ...
    // FIXED: Ensure userId is a number
    const userIdNumber = Number(tokenData.UserID);
    if (isNaN(userIdNumber)) {
