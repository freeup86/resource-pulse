...
const openAITelemetry = require('./openAITelemetry');
...
if (!OPENAI_API_KEY) {
  console.warn('OpenAI API key not configured. AI recommendations will use fallback data.');
} else {
  console.log('OpenAI API key configured. Will attempt to use AI recommendations with rate limiting.');
}

/**
 * Rate limiting configuration for OpenAI API requests
 * These values should be adjusted based on your OpenAI subscription tier and rate limits
 * 
 * - Free tier: 20 requests/minute (RPM), 40k tokens/minute
 * - Lowest paid tier: 60 RPM, 60k tokens/minute
 * - Regular tier: 3500 RPM, 90k tokens/minute
 * 
 * Reference: https://platform.openai.com/docs/guides/rate-limits
 */
const RATE_LIMIT = {
  requestsPerMinute: 10,  // More conservative than the 20 RPM limit to provide a safety margin
  tokensPerMinute: 30000, // Approximate token limit (assuming free tier with safety margin)
  maxConcurrent: 2,       // Reduced to 2 concurrent requests for more stability
  retryDelay: 3000,       // Increased base delay to 3 seconds
  maxRetries: 4,          // Increased max retries
  retryMultiplier: 1.5,   // Use 1.5x multiplier instead of 2x for more gradual backoff
  initialDelay: 500       // Initial delay between requests to avoid burst traffic
};
...
class RequestQueue {
  ...
  estimateTokenUsage(requestData) {
    ...
    // For chat models, roughly 1 token = 4 characters for English text
    let totalChars = 0;
    
    // Count characters in system and user prompts
    ...
    return Math.max(500, Math.ceil(totalChars / 3));
  }
  ...
  async processQueue() {
    ...
    try {
      ...
      openAITelemetry.recordSuccess(result.data);
      ...
      openAITelemetry.recordError(error);
      
      // Handle rate limit errors with exponential backoff
      if (error.response && error.response.status === 429 && requestToProcess.retries < RATE_LIMIT.maxRetries) {
...
async function generateAIRecommendation(skill, project, userContext = {}) {
  try {
    ...
    if (OPENAI_API_KEY) {
      try {
        console.log(`Generating AI recommendation for skill: ${skill.Name} in project: ${project.Name}`);
        
        // Build system prompt with personalization guidance
        const systemPrompt = `
        ...
        if (Object.keys(userContext).length > 0) {
          ...
          Personal context to consider when making recommendations:
        ...
        const userPrompt = `
          Generate a professional learning recommendation for a skill: ${skill.Name}
          ...
          Project context: 
          Name: ${project.Name}
          Description: ${project.Description || `A project requiring ${skill.Name} expertise`}
        ...
        const response = await apiRequestQueue.add(
          async () => {
            console.log(`Executing OpenAI API request for skill: ${skill.Name} (queue size: ${apiRequestQueue.queue.length}, running: ${apiRequestQueue.running})`);
        ...
        if (apiError.response && apiError.response.status === 429) {
          console.error('OpenAI rate limit exceeded. Using fallback recommendation.', {
            status: apiError.response.status,
            headers: apiError.response.headers
          });
          
          // Check for retry-after header to inform retry timing
          const retryAfter = apiError.response.headers['retry-after'];
          if (retryAfter) {
            console.log(`OpenAI suggests retry after ${retryAfter} seconds`);
          }
        }
        throw apiError;
      }
