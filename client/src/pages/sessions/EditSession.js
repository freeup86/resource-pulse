...
import { getSessionById, updateSession, mockSessions } from '../../services/sessionService';
...
const EditSession = () => {
  ...
  useEffect(() => {
    const fetchSessionData = async () => {
      try {
        ...
        setError('Failed to load session data. The session may not exist or there was a network error.');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Handle course change with validation for enrollment
  const handleCourseChange = (e) => {
    const newCourseId = e.target.value;
    const selectedCourse = mockCourses.find(course => course.id === newCourseId);
    
    // Check if changing course would cause enrollment issues
    if (formData.enrollmentCount > 0 && newCourseId !== formData.courseId) {
      // Show warning or confirmation dialog in a real app
      const confirmChange = window.confirm(
        `This session has ${formData.enrollmentCount} enrolled students. Changing the course may affect their eligibility. Continue?`
      );
  ...
  const validateForm = () => {
    ...
    if (!formData.instructorId && !instructor) {
    ...
    if (formData.maxCapacity <= 0) {
      ...
      valid = false;
    } else if (formData.maxCapacity < formData.enrollmentCount) {
      errors.maxCapacity = `Capacity cannot be less than current enrollment (${formData.enrollmentCount})`;
      valid = false;
    }
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={handleChange}
                InputProps={{ 
                  inputProps: { min: formData.enrollmentCount || 1 },
                  startAdornment: (
                    <InputAdornment position="start">
                      Current Enrollment: {formData.enrollmentCount}
                    </InputAdornment>
                  )
                }}
                error={!!formErrors.maxCapacity}
                helperText={formErrors.maxCapacity || `Must be at least ${formData.enrollmentCount || 1}`}
                disabled={submitting}
              />
            </Grid>
